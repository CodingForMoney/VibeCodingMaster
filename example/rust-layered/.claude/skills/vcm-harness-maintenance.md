...
- Inspect harness state, durable docs, role rules, skills, generated-context tools, long-running helper scripts, PR template, generated artifacts, and runtime cleanup candidates.
...
1. Read `CLAUDE.md`, `.claude/agents/*.md`, `.claude/skills/*.md`, `.github/pull_request_template.md`, durable docs, `.ai/tools/*`, `.ai/generated/*`, and `.ai/vcm-harness-manifest.json` when present.
2. Check for stale docs, obsolete file references, duplicate rules, missing owners, obsolete tools, old runtime state, missing manifest entries, and drift from the current VCM harness baseline.
3. When generated-context tools exist, run their `--check` mode or report that freshness could not be verified.
