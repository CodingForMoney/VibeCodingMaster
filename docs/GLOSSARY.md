...
| SQL | Structured Query Language | SQL query, schema, or database behavior. |
| TODO | To Do | Deferred-work code comment marker. |
| UI | User Interface | User-facing interface or presentation state. |
| URL | Uniform Resource Locator | Web or resource address. |
| UTC | Coordinated Universal Time | UTC timestamps in filenames and reports. |
| UUID | Universally Unique Identifier | UUID-formatted identifier. |
| VCM | VibeCodingMaster | VCM-managed harness, workflow, task, role, or runtime concept. |
| XML | Extensible Markup Language | XML data, files, or payloads. |
