...
- VCM 使用同一个翻译通道读取 Claude Code JSONL transcript 中的结构化 assistant prose output，并转成用户语言，帮助用户理解执行过程。
...
- 将 Claude Code transcript 中的 assistant prose output 持续转换为用户语言解释。
- 保留英文原文和翻译结果的对应关系。
...
User writes Chinese in Translation Panel
  ...
  -> VCM tails assistant text events and waits for turn completion
...
- 只对 assistant `text` event 做轻量分类，再决定翻译、摘要、保留原文或跳过。
- 对 `stop_reason=tool_use` 的中间回合不立即翻译，等待 Claude Code 后续 `end_turn` 或其他完成态输出。
...
~/.claude/projects/<project-hash>/<session-id>.jsonl
  ...
  -> translate assistant text only when stop_reason is not tool_use
  -> classify text
       - prose
       - code/diff/log
       - permission prompt
...
- Claude transcript assistant prose 翻译成功率。
...
- 持续处理 Claude Code transcript assistant text event，并按 prose / code / diff / log / permission prompt 分类。
- 每个 role session 使用独立 FIFO queue 翻译 prose event，保持展示顺序。
