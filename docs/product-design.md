...
- VCM 使用同一个翻译通道读取 Claude Code JSONL transcript 中的结构化 assistant output，并转成用户语言或原文辅助视图，帮助用户理解执行过程。
...
- 将 Claude Code transcript 中的 assistant text、question、todo、agent output 持续转换为用户语言解释。
- 将 raw tool_use / tool_result 以 preserved 原文行显示在 Translation Panel，保持和 Claude Code 执行上下文一致。
- 保留英文原文和翻译结果的对应关系。
...
User writes Chinese in Translation Panel
  ...
  -> Claude Code writes semantic JSONL transcript events
  -> VCM tails assistant text / question / todo / agent events
  -> Translation Provider translates Claude prose output to Chinese
...
- `en-to-zh` 必须基于 Claude Code transcript 的结构化内容先分类，再决定翻译、摘要、保留原文或跳过。
...
- 持续 tail Claude Code session JSONL transcript，读取结构化 assistant content event。
- 对 assistant text / question / todo / agent event 做轻量分类，再决定翻译、摘要、保留原文或跳过。
- assistant `text` event 只要出现就进入翻译队列；`stop_reason` 只用于完成状态判断，不用于过滤翻译。
- `AskUserQuestion`、`TodoWrite`、`Agent` / `Task` 结构化事件转换为可读文本后翻译。
- raw `tool_use` / `tool_result` 不调用翻译模型，作为 preserved 原文行进入 Translation Panel。
...
~/.claude/projects/<project-hash>/<session-id>.jsonl
  -> tail appended JSONL lines
  -> parse assistant / user tool_result events
  -> translate assistant text events regardless of stop_reason
  -> translate structured AskUserQuestion / TodoWrite / Agent / Task prose
  -> preserve raw tool_use / tool_result rows without model calls
  -> classify translatable text
       - prose
       - code/diff/log
       - permission prompt
       - error
...
- 默认低 temperature。
- 利用 transcript event boundary 作为输出边界，避免对 raw TUI repaint 做 batch / debounce。
- 对重复内容做去重。
- 对超长输出做摘要式翻译，而不是逐字翻译。
...
- API key 只保存在本机。
- 本机设置统一保存在 `~/.vcm/settings.json`，翻译配置位于 `translation` 字段。
...
- Claude transcript assistant text / question / todo / agent 翻译成功率。
...
- 持续处理 Claude Code transcript assistant text / question / todo / agent event，并按 prose / code / diff / log / permission prompt 分类。
- 每个 role session 使用独立 FIFO queue 翻译 prose event，保持展示顺序；raw tool_use / tool_result 以 preserved 原文行显示。
- 对 code、diff、log、tool output 默认保留原文或摘要，不逐字翻译。
- 对已经是目标语言或 CJK 的 text event 跳过翻译。
...
- 使用 Claude transcript event boundary，避免 raw terminal repaint 触发大量无意义翻译。
- 超长日志默认摘要翻译。
- 重复输出去重。
