...
worktree. Native model choices are `default`, `fable`, `opus`, and `sonnet`;
the latter three resolve through Claude Code. The template also stores task
auto-orchestration preference.
...
backend restart and disabling Auto Memory.
