...
- Workflow-control changes: run `workflow-control-service.test.ts`,
  ...
  evidence rejection, fresh Gate exceptions, identical-content evidence
...
| INT-RT-002 | Post-task memory and harness review order | Final Acceptance + Review Task Harness + `runtime-coordinator-service` + Harness route | A normally stopped complete flow collects optional Auto Memory proposals before one Task Harness Retrospective | With Auto Memory on, workflow-role proposal hooks start and stop a normal Round, then one Harness Engineer turn reviews every proposal item and pending feedback, commits only assigned `<VCM-memory>` blocks, writes the retrospective and strict `review-result.json` directly, and lets VCM validate exact decision coverage; move-to-durable-doc removes memory immediately, dispatches one owner assignment at a time, validates its exact report and commit, survives restart, and delays retrospective completion until all assignments finish; with Auto Memory off, the prompt omits every memory path and creates no assignment | L2, on Auto Memory or retrospective change | Covered by backend E2E with mock role sessions plus Auto Memory, Harness prompt, assignment recovery, artifact, and guard tests |
...
| INT-GATE-005 | Repair source-chain review | Workflow Control history + code-diff Gate + Reviewer | Repair code that enters before the first code-diff is reviewed with every code producer from the current flow run | A Code-Change run that reaches Coder and Architect Debug supplies `coder -> architect-debug`, retains both commits and evidence sets, includes changed regression tests, and reviews every repair hunk as fresh code; standalone repair flows do not inherit unrelated sources | L2, on Gate source-chain or repair-flow change | Covered by `gate-corrections.e2e.test.ts` and Harness template tests |
| INT-HAR-001 | Scaffold ledger lifecycle | `check-scaffold-ledger` + Architect/Coder/PM/Reviewer rules | Scaffold placement and implementation completion use distinct machine invariants | Scaffold mode requires one marker per Manifest item; completion mode accepts all-done zero-marker delivery and mixed done/failed delivery while rejecting report, Decision, action, marker-state, count, and location mismatches | L1, on scaffold ledger or completion-contract change | Covered by `harness-tools.test.ts`, Harness service assertions, and template-sync tests |
...
| INT-RT-006 | PM question hard pause and delivery | `vcm-ask-user` + Workflow Control + Claude hooks + translation + Gateway | The user receives PM's actual final reply and advancement stays locked until direct user input | Registration cancels the pending approval/proposal without synthesizing a reply; CC terminal/transcript and panel/Gateway use the same full reply under every translation scope; translation off preserves the original; success/failure share one translation; Pause Alert never contains the registered question; repeated Stop/panel reload do not duplicate replies or translation; new Gate requests/retries, routing, and completion are blocked; callbacks do not unlock; direct user input requires fresh approval | L2/L3, on PM communication, workflow, reply, translation, Gateway, or Hook change | Workflow-control unit tests, Pause Alert presentation tests, and backend mock-Claude/Gateway E2E |
...
- Restart With Context for all five workflow roles, explicit tool-Agent
  exclusion, and recovery before the replacement's first accepted prompt.
...
- Architecture, validation, and code-diff rejection/correction loops, including
  corrected commit source chains, first-review repair source reconstruction,
