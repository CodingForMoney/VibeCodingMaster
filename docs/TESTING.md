...
| INT-RT-002 | Post-task memory and harness review order | Final Acceptance + Review Task Harness + `runtime-coordinator-service` + Harness route | A normally stopped complete flow collects optional Auto Memory proposals before one Task Harness Retrospective | With Auto Memory on, workflow-role proposal hooks start and stop a normal Round, then one Harness Engineer turn reviews every proposal item and pending feedback, directly updates only assigned `<VCM-memory>` blocks, commits those host files, writes the assigned retrospective Result Path directly without `vcm-artifact`, and lets VCM record the result; with Auto Memory off, that prompt omits every memory path | L2, on Auto Memory or retrospective change | Covered by backend E2E with mock role sessions plus Harness prompt and guard tests |
