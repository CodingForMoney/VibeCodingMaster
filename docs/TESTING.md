...
| E2E-005 | Await-user banner journey (PM stop → captured reply → persistent web banner → clears on answer) | GUI task workspace; backend `roundState.flowPause` via workspace-state | The await-user pause is captured end-to-end and surfaced as a persistent, injection-safe banner that the transient modal does not duplicate | When a user-facing (`project-manager`) role stops with no onward route, a persistent banner shows the role + captured `flowPause.message` (escaped text); `selectFlowPauseAlertMessage` returns null for `awaiting-user` (no double-alert); the banner stays sticky through gate-reviewer/auto-resume and clears only when the backend clears the anchor (PM's next prompt) | L3, on await-user capture/banner/flow-pause change | Not yet implemented as a browser spec; needs a Playwright harness + live `claude`/pty. Until then the journey is covered below L3: backend capture→stash→settle→message + sticky + stale-stash isolation (`round-service.test.ts`), best-effort capture (`claude-hook-service.test.ts`), shared reply helper incl. truncation (`claude-transcript-reply.test.ts`), gateway behavior-neutrality (`gateway-service.test.ts`), and the banner render incl. HTML-escaping + no-double-alert (`await-user-banner.test.ts`, `flow-pause-alert.test.ts`) |
