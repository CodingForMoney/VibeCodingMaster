...
| INT-RT-006 | PM question hard pause and delivery | `vcm-ask-user` + Workflow Control + Claude hooks + translation + Gateway | The user receives the registered question and advancement stays locked until direct user input | Registration cancels the pending approval/proposal; Stop publishes one canonical reply even without transcript text; Pause Alert displays the question with translation off; panel/Gateway share one translation (success or failure); repeated Stop/replay do not duplicate it; new Gate requests/retries, routing, and completion are blocked; callbacks do not unlock; direct user input requires fresh approval | L2/L3, on PM communication, workflow, reply, translation, Gateway, or Hook change | Workflow-control and question-reply unit tests, Pause Alert presentation test, and backend mock-Claude/Gateway E2E |
