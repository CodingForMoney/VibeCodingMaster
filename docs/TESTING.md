...
| E2E-005 | Await-user alert (a user-facing role stops awaiting a user decision → flow-pause modal + alarm sound) | GUI task workspace; backend `roundState.flowPause` (reason `awaiting-user`) via workspace-state | A user-facing role's await-user pause reuses the standard blocking flow-pause modal; sound is independent and repeats while enabled; Gateway does not suppress the modal, and Gateway input successfully submitted to PM dismisses it | When `flowPause.reason === "awaiting-user"`, `selectFlowPauseAlertMessage` returns the generic modal wording ("No new turn started after project-manager stopped."); sound mode is `strong` whenever the preference is enabled; the latest successful Gateway-to-PM input ID is deduplicated before requesting dismissal | L3, on await-user / flow-pause-alert change | Not yet implemented as a browser spec; covered below L3 by `flow-pause-alert.test.ts` and Gateway backend E2E status assertions |
