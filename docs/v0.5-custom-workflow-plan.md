...
Status: deferred design proposal. The current VCM runtime still uses the fixed
workflow and does not implement the workflow-definition or workflow-run model
described below. The current PM-declared advisory task state is recoverable
context for the fixed workflow; it is not the workflow engine proposed here.
...
```json
{
  ...
  "description": "Default PM -> Architect -> Coder -> Tester flow.",
  "roles": {},
  "nodes": [],
  "edges": [],
  "variables": {},
...
- `review-heavy-workflow`: stricter gates around architecture and code diff
...
- Tester often benefits from fresh context and independent judgment.
- A reviewer often benefits from narrow context and strict artifact input.
...
- roles that may edit tests and fixtures but not production code
...
- Tester can edit tests, fixtures, test-only helpers, and test reports, but not
  production code.
...
### Step 1: Workflow Engine Foundation
...
### Step 2: Role and Session Customization
...
### Step 4: Advanced Gates and Branching
...
### Step 5: Sharing and Auditability
