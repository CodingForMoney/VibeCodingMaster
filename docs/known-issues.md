...
### KI-016 — CCR-backed Claude sessions can miss Hook activity updates

- **Status**: Open.
- **Category**: Product / correctness.
- **Affected modules / surfaces**: CCR-backed role sessions, Claude Code Hooks, Session and Round activity state.
- **Current gap**: Some GPT-5.6 Sol sessions remain `idle` while a turn is running because VCM does not receive or record the expected `UserPromptSubmit` Hook. Claude Code documentation and a local CLI probe confirm that the CCR-only inline `--settings` value merges with project settings and that the isolated `CLAUDE_CONFIG_DIR` does not inherently disable project Hooks, so the failure point remains unconfirmed.
- **Impact**: VCM can display incorrect role activity and derive incorrect Round state while the GPT-backed role is working.
- **Mitigation / workaround**: None confirmed.
- **Resolution condition**: Reproduce the failure while capturing Claude Hook loading, Hook command execution, HTTP delivery, and backend Session matching, then fix the first failed boundary and cover the GPT Session `UserPromptSubmit -> running -> Stop -> idle` lifecycle.
- **Related**: None.
