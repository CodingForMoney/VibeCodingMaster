...
\`create\`/\`change\`/\`delete\` entry (an \`asset\` entry carries no marker). This tool checks:
  ...
  1. the ledger header follows the mandated column order (\`ID | Action | File | ...\`),
     ...
     (create/change/delete/asset), and — except for \`asset\` — bound to a declared file
     path in the file column;
  ...
  4. \`asset\` entries have no marker in the tree;
  5. the manifest contains no open-ended coverage language ("as work proceeds",
...
ACTIONS = frozenset({"create", "change", "delete", "asset"})
    ...
    for offset, line in enumerate(section):
        ...
        if action is None:
            findings.append(
                ...
                f"one of create/change/delete/asset -> classify the item in column 2"
            )
        if action != "asset" and declared is None:
            findings.append(
...
def main() -> int:
    ...
    for entry_id in sorted(ledger_ids - tree_ids):
        if entries[entry_id]["action"] != "asset":
            findings.append(
    ...
    for entry_id in sorted(ledger_ids & tree_ids):
        ...
        if entry["action"] == "asset":
            path, line_no = markers[entry_id][0]
            findings.append(
                f"{path}:{line_no}  [ledger] asset entry {entry_id} must not carry a marker"
            )
        elif entry["path"]:
            declared = entry["path"]
