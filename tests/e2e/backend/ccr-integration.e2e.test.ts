...
import { VCM_CODE_INTELLIGENCE_PLUGIN_DIR } from "../../../src/backend/services/lsp-plugin.js";
...
describe("backend E2E CCR integration", () => {
  ...
  it("keeps native launches clean and blocks future CCR launches after disabling", async () => {
    ...
    expect(nativeInput.args).toEqual(expect.arrayContaining(["--plugin-dir", VCM_CODE_INTELLIGENCE_PLUGIN_DIR]));
    expect(nativeInput.env.VCM_CODE_INTELLIGENCE_ENABLED).toBe("true");
  ...
  it("uses the same CCR launch path for Reviewer and auxiliary sessions", async () => {
    ...
    expect(reviewerInput.args).toEqual(expect.arrayContaining(["--plugin-dir", VCM_CODE_INTELLIGENCE_PLUGIN_DIR]));
    expect(reviewerInput.env.VCM_CODE_INTELLIGENCE_ENABLED).toBe("true");
    for (const auxiliary of launches.slice(1)) {
      ...
      expect(auxiliaryInput.env.VCM_CODE_INTELLIGENCE_ENABLED).toBeUndefined();
    }
  });
